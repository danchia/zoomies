...
class Localizer {
 public:
  ...
  void OdoFrame(int64_t t_us, float x, float y, float heading);
...
void Localizer::InitViz() {
  ros_writer_ = McapLogWriter::Make("/tmp/mapviz");
...
void Localizer::VideoFrame(int64_t t_us, const std::vector<uint8_t>& img) {
  ...
  map_maker_.Update(Eigen::Vector3f{x_, y_, heading_}, rgb_img);
...
void Localizer::OdoFrame(int64_t t_us, float x, float y, float heading) {
  x_ = x;
  y_ = y;
  heading_ = heading;
}
...
int main() {
  ...
  for (const auto& msgView : messages) {
    ...
    if (channel.topic == "/camera1/raw") {
      ...
      t_us /= int64_t{1000};
      spdlog::info("{} {}", msgView.message.publishTime, t_us);
      ...
      int64_t t_us = msgView.message.publishTime;
      t_us /= int64_t{1000};
      localizer.OdoFrame(t_us, m.x(), m.y(), m.heading());
      ++datas;
    }
  }
